...
impl T3HeightMap {
    ...
    pub fn parse<'a>(input: &'a [u8], map_info: &MapInfo) -> BevySC2MapResult<&'a [u8], Self> {
        ...
        if terrain_dimensions != map_info_dimensions {
            return Err(BevySC2MapError::T3HeightDimDoNotMatchMapInfoDim(
                terrain_dimensions,
                map_info_dimensions,
            ));
        }
        ...
        if tail.len() < expected_terrain_data_size {
            return Err(BevySC2MapError::T3HeightNotEnoughBytes(
...
mod tests {
    ...
    fn test_parse_map_info() {
        ...
        if let Err(BevySC2MapError::T3HeightNotEnoughBytes(171366, 168)) = t3_height_map {
            // Expected error
        } else {
            panic!(
