...
impl MapInfo {
    ...
    pub fn parse(input: &[u8]) -> BevySC2MapResult<&[u8], Self> {
        ...
        let (tail, width_bytes) = dbg_peek_hex(take(4usize), "read map width, 4 bytes")(tail)?;
        let (_, width) = u32(nom::number::Endianness::Little)(width_bytes)?;

        let (tail, height_bytes) = dbg_peek_hex(take(4usize), "read map height, 4 bytes")(tail)?;
        let (_, height) = u32(nom::number::Endianness::Little)(height_bytes)?;

        if width > 256 || height > 256 {
            tracing::warn!(
                "MapInfo width({}) or height({}) is larger than expected 256",
                width,
                height
            );
            return Err(BevySC2MapError::InvalidMapSize(width.max(height)));
        }
        let (tail, _unknown_bytes) =
            dbg_peek_hex(take(8usize), "read 8 unknown bytes after height")(tail)?;
        let (tail, _) = dbg_peek_hex(
            take_while(|x| x == 0u8),
            "padding zeros fill before Strings after height+unknown",
        )(tail)?;
        ...
        tracing::info!("MapInfo ------ Next: {}", peek_hex(tail));
        Ok((
            tail,
            Self {
                file_version,
...
mod tests {
    ...
    fn test_parse_map_info() {
        ...
        let cache_contents: Vec<u8> = vec![
            ...
            0x00, // Advance termination character
            0x0e, 0x00, 0x00, 0x00, 0x0e, 0x00, 0x00, 0x00, 0x9a, 0x00, 0x00, 0x00, 0x9a, 0x00,
            ...
            0x00, 0x20, 0x03, 0x00, 0x00, 0x58, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x20,
            0x00, 0x00, 0xbd, 0xc6, 0x0a, 0x68, 0xa7, 0xc6, 0x0a, 0x68, 0x10, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00,
            ...
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        assert_eq!(map_info.width, 168);
        assert_eq!(map_info.height, 168);
